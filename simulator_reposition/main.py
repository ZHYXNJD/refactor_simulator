            ...
            for epoch in range(NUM_EPOCH):
                ...
                # GET STATE
                    ...
                    if len(grid_array) > 0:
                        ...
                        index_grid = np.where(grid_array.reshape(grid_array.size, 1) == simulator.zone_id_array)[1]
                ...
                logger.debug(f"epoch: {epoch} == total_reward:{simulator.total_reward},matching_rate:{simulator.matched_requests_num/simulator.total_request_num},total_request_num:{simulator.total_request_num},matched_request_num:{simulator.matched_requests_num},occupancy_rate_repo:{simulator.occupancy_rate_repo},occupancy_rate:{simulator.occupancy_rate},occupancy_rate_no_pickup:{simulator.occupancy_rate_no_pickup},wait_time:{simulator.waiting_time / simulator.matched_requests_num},pickup_time:{simulator.pickup_time / simulator.matched_requests_num}")
                if epoch > 100 and epoch % 20 == 0:  
                    repo_agent.save_model(epoch, model_timestamp)
