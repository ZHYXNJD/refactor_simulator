...
版本控制：2026-08-25 已将本轮源码、测试、启动脚本和文档提交为`5dd767c`（`Implement H1 COMA stabilization and scenario tooling`）；交付远程为`refactor_simulator/main`，随后已推送。曾额外推送到`origin/main`（`Transportation_Simulator`）是误选远程，不改变正确交付已在`refactor_simulator/main`的事实。明确未提交/未推送`dynamic_matching/qtable_*`、`queens_local_smoke_results/`及`c35_paired_intervention_candidates.json`等数据、模型和实验结果工件；它们仍仅保留在本地未跟踪状态。
