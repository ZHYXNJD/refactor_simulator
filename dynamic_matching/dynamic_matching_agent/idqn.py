...
class IDQN:
    def __init__(
        ...
        self.tau = 0.005
        self.buffer_size = HYPERPARAMS.get('buffer_size',5000)
        ...
        self.grid_num = HYPERPARAMS['grid_num']
        self.decision_freq = HYPERPARAMS['decision_freq']
        ...
        if transitions is not None:
            ...
            if self.load_offline_warmup:
                warmup_data_file = f"./dynamic_matching_algorithm/warmup_transitions/idqn/grid_{self.grid_num}_freq_{self.decision_freq}/transition_data_brand_new.pkl"
                scaler_file = f"./dynamic_matching_algorithm/warmup_transitions/idqn/grid_{self.grid_num}_freq_{self.decision_freq}/transition_data_state_scaler_brand_new.pkl"
        ...
        # --- derive per-agent local input dim from provided obs_dims for backwards compatibility ---
        try:
            total_obs_with_onehot = obs_dims[0]
            total_state_dim = total_obs_with_onehot - self.n  # remove one-hot part
            # local features per grid (e.g., wait/idle/occupied)
            self.local_feature_len = int((total_state_dim - 2) // self.n)
            # per-agent input: local features + time encoding (2)
            self.local_input_dim = self.local_feature_len + 2
        except Exception:
            # fallback: use provided obs_dims[i]
            self.local_input_dim = obs_dims[0]
            self.local_feature_len = max(0, obs_dims[0] - 2)

        for i in range(self.n):
            input_dim = self.local_input_dim  # use local state only (local features + time)
    ...
    def select_actions(self, global_state, deterministic=False):
        ...
        with torch.no_grad():
            for i in range(self.n):
                # 构造输入: 使用 local state（该 agent 对应 grid 的特征） + 全局 time encoding
                # global_state: [state_dim], state_dim = grid_num * local_feature_len + 2 (time)
                start = i * self.local_feature_len
                end = start + self.local_feature_len
                local_feat = global_state[start:end]
                time_enc = global_state[-2:]
                agent_input = torch.cat([local_feat, time_enc], dim=-1).unsqueeze(0)  # [1, input_dim]
    ...
    def update(self):
        ...
        if not self.is_scaler_fitted:
            ...
            self.is_scaler_fitted = True
            self.warmup_states = []
            write_path = 'warmup_transitions/IDQN'
            if not os.path.exists(write_path):
                os.makedirs(write_path)

            with open(write_path + f'/grid_{self.grid_num}_freq_{self.decision_freq}_state.pkl', 'wb') as f:
                pickle.dump(list(self.buffer.buffer), f)  # 假设 buffer.buffer 是你的deque
                # **(关键)** 保存拟合好的 Scaler
            joblib.dump(self.state_scaler, write_path + f'/grid_{self.grid_num}_freq_{self.decision_freq}_state_scaler.pkl')
        ...
        for _ in range(self.update_num):
            ...
            for i in range(self.n):
                # --- Construct Inputs using local state only ---
                # global_state: [batch, total_state_dim], total_state_dim = n * local_feature_len + 2
                start = i * self.local_feature_len
                end = start + self.local_feature_len
                local_batch = global_state[:, start:end]  # [batch, local_feature_len]
                next_local_batch = next_global_state[:, start:end]
                time_batch = global_state[:, -2:]  # [batch, 2]
                next_time_batch = next_global_state[:, -2:]

                curr_state_input = torch.cat([local_batch, time_batch], dim=-1)  # [batch, input_dim]
                next_state_input = torch.cat([next_local_batch, next_time_batch], dim=-1)
    ...
    def load(self, grid_num=None, decision_freq=None, path=None):
        """
        Trainer compatibility:
          - trainer/test calls: load(grid_num, decision_freq)
          - legacy calls still work: load(path="...") or load(".../model.pt")
        """
        # legacy: load("path")
        if path is None and isinstance(grid_num, (str, bytes, bytearray)):
            path = grid_num
            grid_num, decision_freq = None, None
        ...
        if grid_num is not None and decision_freq is not None:
